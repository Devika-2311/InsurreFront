...
function PurchaseDetails() {
  ...
  const handleStatusUpdate = async (status) => {
    try {
      const response = await axios.put(`http://localhost:7001/admin/purchases/${selectedPurchase.policyDetailsId}`, status, {
        headers: {
  ...
  return (
    <div className="purchase-details">
      ...
      <button onClick={() => {handleStatusUpdate('Approved'); sendEmail();}}className="approve-button">Approve</button>
      <button onClick={() => {handleStatusUpdate('Rejected');sendEmail();}} className="reject-button">Reject</button>
      </div>
    </div>
  );
}
